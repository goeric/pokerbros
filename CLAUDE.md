...
**Tech Stack:**
- Frontend: React/Next.js 14 with TypeScript and App Router
...
7. **Admin Controls**: Only admin users can add rebuys, end games, and modify game data
...
3. **Cache staleness**: Always call `revalidatePath()` in Server Actions after mutations
...
10. **Live game sync**: When initializing game_players from RSVPs, always sync on page load to catch new players who RSVP'd after initial setup
...
Google OAuth via Supabase Auth. Admin users must exist in `admin_users` table.

**Key Points:**
- Always use `localhost` (not `127.0.0.1`) for local development
- Use `createBrowserClient` for client-side, `createServerClient` for server-side
